...
cur_path = "G:/Mi unidad/doctorado/Pablo Castillo - DOCTORADO/2_soa/8 Prepare scenario/Audio analysis/LocalPython/Prototipo Whisperx_/structured outputs/"
...
filename = f"merged_orders_{scenario}.json"
...
with open(f"simplified_orders_merged_{scenario}.json", "w") as f:
...
unique_skills = {}
filename = f"merged_skills_{scenario}.json"
with open(os.path.join(cur_path, filename), "r") as f:
    data = json.load(f)
...
# Optionally save to a new file
with open(f"simplified_skills_merged_{scenario}.json", "w") as f:
    json.dump(timelineList, f, indent=2)
